...
{
    fprintf(stderr,
        "tx_samples_c - A simple TX example using UHD's C API\n\n"
...
int main(int argc, char* argv[])
{
    ...
    uhd_usrp_get_pp_string(usrp, usrp_info, 500);

    fprintf(stderr, "Thong tin chung cua USRP \"%s\"...\n", usrp_info);
    ...
    // Set up buffer
    EXECUTE_OR_GOTO(free_tx_streamer, uhd_tx_streamer_max_num_samps(tx_streamer, &samps_per_buff))    //get maximum number of sample per buffer
    ...
    while (1) 
    {
        ...
        EXECUTE_OR_GOTO(free_buff,uhd_tx_streamer_send(tx_streamer, buffs_ptr, samps_per_buff, &md, 0.1, &num_samps_sent))
